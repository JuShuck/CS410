    ...
    public void dump(AbstractAirline a) throws IOException {
        ...
        try {
            ...
            for(Flight f : list) {
                ...
                String departure = formattingFormat.format(parseFormat.parse(f.getDepartureString()));
                String arrival = formattingFormat.format(parseFormat.parse(f.getArrivalString()));

                out.println(a.getName() + " "+ f.getNumber() + " " + f.getSource() + " " + departure + " " + f.getDestination() + " " + arrival);
            }
            out.close();
